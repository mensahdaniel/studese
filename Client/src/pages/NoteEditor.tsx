...
const NoteEditor = () => {
  ...
  useEffect(() => {
    const fetchUserAndNote = async () => {
      ...
      if (id !== "new") {
        // First try to fetch the note - RLS will handle access control
        // Don't filter by user_id to allow viewing shared notes
        // Check if user has a share record for this note (to determine permission)
        const { data: userShareData } = await supabase
          .from("note_shares")
          .select("permission")
          .eq("note_id", id)
        ...
        if (error || !data) {
          // Check if this is a shared note the user has access to
          ...
          if (!shareData) {
            toast({ title: "Error", description: "Note not found or you don't have access.", variant: "destructive" });
            navigate("/notes");
            return;
          }

          // If we have share access, try fetching via the share relationship
          const { data: sharedNoteData, error: sharedError } = await supabase
            .from("note_shares")
            .select(`
              permission,
              notes (*)
          ...
          if (sharedError || !sharedNoteData?.notes) {
            toast({ title: "Error", description: "Note not found or you don't have access.", variant: "destructive" });
            navigate("/notes");
            return;
          }

          const noteData = sharedNoteData.notes as any;
          const permission = sharedNoteData.permission as "view" | "edit";

          // Set permission state for shared note
          setIsOwner(false);
          ...
          if (noteData.canvas_state) {
            setInitialCanvasState(noteData.canvas_state);
            setTemplate(noteData.canvas_state.template || "blank");
            setBackground(noteData.canvas_state.background || "#ffffff");
          }
        ...
        // Check if user owns this note or has edit permission via share
        const noteIsOwned = data.user_id === user.id;
        setIsOwner(noteIsOwned);

        if (!noteIsOwned && userShareData) {
          // User has access via share
          setSharePermission(userShareData.permission as "view" | "edit");
          setCanEdit(userShareData.permission === "edit");
        } else if (noteIsOwned) {
          // User owns the note
          setCanEdit(true);
          setSharePermission(null);
        } else {
          // Fallback - check link permission
  ...
  // Handle canvas changes - trigger auto-save timer (only if user can edit)
  const handleCanvasChange = useCallback((state: CanvasState) => {
    // Don't track changes or auto-save if user can't edit
    if (!canEdit) return;
  ...
  const handleSave = async () => {
    if (!canEdit) {
      toast({ title: "View Only", description: "You don't have permission to edit this note.", variant: "destructive" });
      return;
    }
  ...
  // Share functionality - only owners can share
  const handleShare = () => {
    if (!isOwner) {
      toast({
        title: "Cannot Share",
        description: "Only the note owner can share this note.",
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="h-dvh bg-background flex flex-col overflow-hidden">
        ...
        <header className={`h-12 sm:h-14 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 flex items-center justify-between px-2 sm:px-4 z-50 shrink-0 transition-all duration-300 ${isZenMode && !zenHeaderActive ? "opacity-0 pointer-events-none absolute top-0 left-0 right-0" : ""}`}>
            ...
            <div className="flex items-center gap-1 sm:gap-2 ml-1 sm:ml-2 min-w-0 flex-1">
              ...
              {isEditingTitle && canEdit ? (
                ...
                <button
                  ...
                  className={`font-medium text-xs sm:text-sm max-w-[100px] sm:max-w-[200px] truncate px-1.5 sm:px-2 py-1 rounded transition-colors text-left ${canEdit ? "hover:bg-muted cursor-pointer" : "cursor-default"}`}
                  title={canEdit ? "Click to rename" : "View only"}
                >
                  {title || "Untitled Note"}
                </button>
              )}
              {/* View Only Badge */}
              {!canEdit && !isOwner && (
                <Badge variant="secondary" className="h-5 text-xs flex bg-blue-500/90 text-white">
                  <Eye className="h-3 w-3 mr-1" />
                  View Only
                </Badge>
              )}
              {/* Can Edit Badge for shared notes */}
              {canEdit && !isOwner && (
                <Badge variant="secondary" className="h-5 text-xs flex bg-green-500/90 text-white">
          ...
          <div className="flex items-center gap-0.5 sm:gap-2 shrink-0">
            {/* Save Status - Icon only on mobile (only show if user can edit) */}
            {canEdit && (
            ...
            <div className="hidden sm:flex items-center gap-1">
              ...
              {/* Settings Sheet - Only show for owners */}
              {isOwner && (
        ...
        <main className="flex-1 relative overflow-hidden">
          ...
          {/* Floating Toolbar - Hidden in Zen Mode unless toggled, hidden for view-only users */}
          {canEdit ? (
            ...
            <div className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-40 pb-safe">
