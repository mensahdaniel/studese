...
const Tasks = () => {
  ...
  const handleAddTask = async () => {
    if (!newTask.title || !newTask.due_date) {
      toast({
        ...
        description: "Title and due date required.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (isNaN(dueDate.getTime())) {
      toast({
        title: "Error",
        description: "Invalid date/time combination.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (error) {
      ...
      const addedTask = {
        ...data[0],
        dynamic_priority: calculateDynamicPriority(data[0].due_date),
      };
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    const matchesFilter =
      selectedFilter === "all" ||
      task.dynamic_priority === selectedFilter ||
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-orange-600">
                {
                  tasks.filter(
                    (t) => new Date(t.due_date) < new Date() && !t.completed
                  ).length
                }
              </div>
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex gap-2">
                          <Badge
                            variant="outline"
                            className={getPriorityColor(task.dynamic_priority)}
                          >
